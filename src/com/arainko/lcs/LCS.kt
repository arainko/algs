...
fun lcsLength(verticalString: String, horizontalString: String): ElementArray {
    ...
    for (i in 1 .. verticalString.length)
        for(j in 1 .. horizontalString.length) {
            ...
            } else if (elements[i-1][j] >= elements[i][j-1]) {
...
fun printLcs(verticalString: String, horizontalString: String, lcsArray: ElementArray) {
    fun helper(verticalString: String, horizontalString: String, lcsArray: ElementArray, i: Int, j: Int) {
        if (i == 0 || j == 0)
            return

        if (lcsArray[i][j].sign == Sign.DIAGONAL) {
            helper(verticalString, horizontalString, lcsArray, i-1, j-1)
            print(verticalString[i])
        }
        else if (lcsArray[i][j].sign == Sign.VERTICAL) helper(verticalString, horizontalString, lcsArray, i-1, j)
        else if (lcsArray[i][j].sign == Sign.HORIZONTAL) helper(verticalString, horizontalString, lcsArray, i, j-1)
    }
    helper(verticalString, horizontalString, lcsArray, horizontalString.length, verticalString.length)
}



fun main() {
    val testString1 = "acbcf"
    val testString2 = "abcdaf"
